...
These are the real thing, generated by [pyXDSM](https://github.com/mdolab/pyXDSM) from
`docs/xdsm/build_xdsm.py`.
...
![XDSM of the baseline set](docs/xdsm/xdsm_baseline.png)

**With an openavl vortex lattice** — seven components, fourteen connections. Two blocks appear, and
the connection carrying `C_L, q, M, h` runs *into* the loads model while only `D` comes back.

![XDSM of the openavl set](docs/xdsm/xdsm_openavl.png)

**Through OpenConcept's own OpenAeroStruct lattice** — structurally identical, one block renamed.
That sameness is the abstraction working, and it is what makes comparing the two codes meaningful.

![XDSM of the OpenAeroStruct set](docs/xdsm/xdsm_openaerostruct.png)

<details>
<summary>The same three as mermaid, for a quick read</summary>
